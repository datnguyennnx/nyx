---
name: mas
description: Use when orchestrating multi-agent shipping pipelines — decomposing work into parallel tasks, spawning agents level-by-level from evidence-gated dependency graphs, and verifying output with a binary compilation gate. Covers complexity scoring via Shannon entropy, Kahn-scheduled DAG execution, evidence coupling, fixer bounded retry, and HITL feedback classification. Use ONLY for MAS orchestration — not for single-agent tasks or direct code edits.
---
...
**Fix:** Run `complexity-score.mjs`. If it throws with file overlap, add a `P6` edge (same-file → sequential) or re-split the tasks so they touch disjoint files.
...
2. It detects cycles in edges. A `P3` edge A→B and another edge B→A looks like a cycle — the script catches it; mental scheduling would deadlock at spawn time.

**Rule:** Always run the script. If it throws, your mental model was wrong.

**Trap 2: Treating "no evidence" as "parallel-safe"**

Discovery returns "no coupling found" for a pair. You classify it as `P5` (parallel-safe). But `P5` requires a **positive confirmation of absence**, not absence of evidence. If discovery never checked that pair, the default is sequential.

**Rule:** Ambiguous = sequential. Only explicit `P5` = parallel.
...
| Throws: "File overlap between same-level tasks" | Two tasks declared the same file | Add P6 edge or re-split tasks |
| Throws: "Cycle detected in DAG edges" | Edges form a cycle (A→B→A) | Re-check P1-P4 directions; they must be acyclic |
| Returns `fastLane: true` | C_T < 0.25, single task | Skip evidence, go straight to implementer |
